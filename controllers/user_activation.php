<?php
    ...
    if($token != "") {
        $sqlQuery = mysqli_query($connection, "SELECT * FROM new_user WHERE token = '$token' ");
        ...
        if($countRow == 1){
            while($rowData = mysqli_fetch_array($sqlQuery)){
                $is_active = $rowData['is_active'];
                  if($is_active == 0) {
                     $update = mysqli_query($connection, "UPDATE new_user SET is_active = '1' WHERE token = '$token' ");
                       if($update){
