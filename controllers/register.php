<?php
    ...
    if(isset($_POST["submit"])) {
        ...
        $email_check_query = mysqli_query($connection, "SELECT * FROM new_user WHERE email = '{$email}' ");
            ...
            if($rowCount > 0) {
                ...
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    // Query
                    $sql = "INSERT INTO new_user (firstname, lastname, email, mobilenumber, password, token, is_active,
